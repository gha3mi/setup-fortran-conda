...
async function setUlimits() {
    ...
    (0,fs__WEBPACK_IMPORTED_MODULE_3__.appendFileSync)(
        process.env.GITHUB_ENV,
        `BASH_ENV=${process.env.RUNNER_TEMP}/ulimit.sh${os__WEBPACK_IMPORTED_MODULE_4__.EOL}`
    );
    (0,fs__WEBPACK_IMPORTED_MODULE_3__.appendFileSync)(
        `${process.env.RUNNER_TEMP}/ulimit.sh`,
        `${ulimitCmd}${os__WEBPACK_IMPORTED_MODULE_4__.EOL}`
    );
...
async function exportActivatedCondaEnv(envName) {
    const scriptPath = path__WEBPACK_IMPORTED_MODULE_2__.join(os__WEBPACK_IMPORTED_MODULE_4__.tmpdir(), 'export-conda-env.sh');

    (0,_actions_core__WEBPACK_IMPORTED_MODULE_0__/* .startGroup */ .Oh)('setup-fortran-conda: Export Conda Environment');
    fs__WEBPACK_IMPORTED_MODULE_3__.writeFileSync(scriptPath, `#!/usr/bin/env bash
set -eo pipefail
