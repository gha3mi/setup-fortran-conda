...
// 🆕 Export all Conda environments to GITHUB_ENV (Windows)
async function exportAllCondaEnvsToGitHubEnvWindows() {
  let raw = '';
  await _exec('conda', ['env', 'list', '--json'], {
    silent: true,
    listeners: { stdout: d => (raw += d.toString()) },
  ...
  for (const prefix of envs) {
    const envName = prefix.split(sep).pop();
    if (!envName || envName.toLowerCase() === 'miniconda') continue;
    ...
    if (code !== 0) {
      info(`⚠️ Skipping environment '${envName}' at ${prefix} (activation failed).`);
      continue;
    }
    ...
    output.split(/\r?\n/).forEach(line => {
      ...
      if (/^(GITHUB_ENV|GITHUB_PATH|RUNNER_TEMP|RUNNER_TOOL_CACHE)$/.test(key)) return;
      const value = rest.join('=').trim();
      env[key] = value;
      appendFileSync(envFile, `${key}=${value}${EOL}`);
    });

    info(`✅ Exported environment from '${envName}'`);
  }
}
...
export async function setup(version = '') {
  ...
  const packages = [gfortranPkg, 'binutils'];

  await runVcvars64(); // 🛠 MSVC environment

  startGroup('Conda install (Fortran)');
  await _exec('conda', [
    'install',
    '--yes',
    '--name', 'fortran',
  ...
  // ✅ Export all environments (fortran included)
  await exportAllCondaEnvsToGitHubEnvWindows();
}
